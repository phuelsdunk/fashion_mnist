...
class SimpleLayer(tf.keras.Model):
    def __init__(self, filters):
        super(SimpleLayer, self).__init__()
        self.conv = tf.keras.layers.Conv2D(filters, (3, 3), padding='SAME')
        self.bn = tf.keras.layers.BatchNormalization()
    def __call__(self, inputs, training=False):
        return tf.nn.relu(self.bn(self.conv(inputs), training=training))

class SimpleDownLayer(tf.keras.Model):
    def __init__(self, filters):
        super(SimpleDownLayer, self).__init__()
        self.conv = tf.keras.layers.Conv2D(filters, (3, 3), strides=(2, 2), padding='SAME')
        self.bn = tf.keras.layers.BatchNormalization()
    def __call__(self, inputs, training=False):
        return tf.nn.relu(self.bn(self.conv(inputs), training=training))
...
def construct_model():
    ...
    model.add(SimpleDownLayer(32))
    model.add(SimpleLayer(32))
    model.add(SimpleDownLayer(64))
    model.add(SimpleLayer(64))
